...
# HRNet backbone with switchable per-branch attention (vanilla SE/CBAM with sigmoid gates).
...
# ----------------------------
# Vanilla attention gates (sigmoid)
# ----------------------------

class SEGate(nn.Module):
    """Squeeze-Excite (vanilla). y = x * sigmoid(MLP(gap(x)))."""
...
class CBAMGate(nn.Module):
    """CBAM-like channel + spatial (vanilla, multiplicative sigmoid gates)."""
...
class SerialAttn(nn.Module):
    """Channel gate -> Spatial gate (both vanilla, multiplicative sigmoid)."""
    def __init__(self, c: int, r: int = 16, k: int = 7):
        super().__init__()
        m = max(8, c // max(1, r))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ca = torch.sigmoid(self.ca(x))
        x = x * ca
        avg = x.mean(1, keepdim=True)
        mx, _ = x.max(1, keepdim=True)
        sa = torch.sigmoid(self.sa(torch.cat([avg, mx], dim=1)))
        x = x * sa
        return x
